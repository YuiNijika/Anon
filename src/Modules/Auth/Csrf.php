...
class Csrf
{
    ...
    private static function isStatelessEnabled(): bool
    {
        if (self::$statelessEnabled === null) {
            if (Env::isInitialized()) {
                self::$statelessEnabled = Env::get('app.base.security.csrf.stateless', true);
            } else {
    ...
    public static function verify(?string $token = null, bool $throwException = true): bool
    {
        ...
        if (empty($token)) {
            if ($throwException) {
                Common::Header(403);
                ResponseHelper::forbidden('CSRF Token 未提供');
            }
            return false;
        }
        ...
        if (!isset($_SESSION['csrf_token'])) {
            if ($throwException) {
                Common::Header(403);
                ResponseHelper::forbidden('CSRF Token 已过期，请刷新页面重试');
            }
            return false;
        }
        ...
        if (!hash_equals($_SESSION['csrf_token'], $token)) {
            if ($throwException) {
                Common::Header(403);
                ResponseHelper::forbidden('CSRF Token 验证失败');
            }
            return false;
        }
    ...
    private static function verifyStatelessToken(string $token, bool $throwException = true): bool
    {
        try {
            ...
            if ($decoded === false) {
                if ($throwException) {
                    Common::Header(403);
                    ResponseHelper::forbidden('CSRF Token 格式错误');
                }
                return false;
            }
            ...
            if (!is_array($data) || !isset($data['timestamp']) || !isset($data['nonce'])) {
                if ($throwException) {
                    Common::Header(403);
                    ResponseHelper::forbidden('CSRF Token 数据错误');
                }
                return false;
            }
            ...
            if (session_status() === PHP_SESSION_ACTIVE && isset($data['session_id'])) {
                ...
                if ($currentSessionId && $data['session_id'] !== $currentSessionId) {
                    if ($throwException) {
                        Common::Header(403);
                        ResponseHelper::forbidden('CSRF Token Session 不匹配');
                    }
                    return false;
                }
            }
            ...
            if ($throwException) {
                Common::Header(403);
                ResponseHelper::forbidden('CSRF Token 验证异常');
            }
            return false;
        }
    }
    ...
    {
        if (Env::isInitialized()) {
            return Env::get('app.base.security.csrf.enabled', true);
        }
