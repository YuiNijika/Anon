...
class StaticResource
{
    ...
    public static function registerRoute(string $route, $filePath, $mimeType = 'application/octet-stream', int $cacheTime = 315360000, array $meta = [])
    {
        ...
        try {
            Config::addRoute($route, function () use ($filePath, $mimeType, $cacheTime) {
            ...
            // 记录路由注册成功
            Debug::info('Static resource route registered', [
                'route' => $route,
                'file_path' => is_string($filePath) ? $filePath : 'callable',
                'mime_type' => is_string($mimeType) ? $mimeType : 'callable'
            ]);
        } catch (Exception $e) {
            Debug::error('Failed to register static resource route', [
                'route' => $route,
    ...
    {
        $themeDir = Main::APP_DIR . 'Theme/' . $themeName . '/';
        if (!empty($basePath)) {
