...
class Attachment
{
    ...
    public static function index()
    {
        ...
        if (empty($fileType) || empty($filename)) {
            Debug::warn('Missing required parameters', [
                'filetype' => $fileType,
                'filename' => $filename
            ]);
            Anon::error('Invalid parameters', null, 400);
        }
        ...
        if (!$attachment) {
            Anon::error('Attachment not found', null, 404);
        }

        $filePath = self::getRealFilePath($attachment, $fileType);

        if (!is_file($filePath) || !is_readable($filePath)) {
            Anon::error('File not found or not readable', null, 404);
        }
    ...
    public static function getAttachment(int $id): ?array
    {
        try {
            ...
            Debug::error('Failed to get attachment', ['id' => $id, 'error' => $e->getMessage()]);
            return null;
        }
    }
    ...
    public static function upload(string $field = 'file', string $fileType = 'other'): array
    {
        ...
        // 生成唯一文件名
        $uploadRoot = Main::APP_DIR . 'Upload/';
    ...
    private static function saveAttachment(array $data)
    {
        try {
            ...
            Debug::error('Failed to save attachment', ['error' => $e->getMessage()]);
            return false;
        }
    }
    ...
    {
        $baseUrl = Env::get('app.base.router.prefix', '/');
    ...
    private static function findAttachment(array $conditions): ?array
    {
        try {
            ...
            Debug::error('Failed to query attachment', [
    ...
    private static function serveWithStrongCache(string $filePath, array $attachment, string $imgType = '')
    {
        ...
        StaticResource::serveFile($filePath, $mimeType, 315360000, false);
    }
    ...
    private static function serveConvertedImage(string $filePath, string $imgType, array $attachment)
    {
        ...
        if (!$targetFormat) {
            Anon::error('Invalid image processing parameters', null, 400);
        }
        
        if (($width || $height) && ($width < 1 || $width > 5000 || $height < 1 || $height > 5000)) {
            Anon::error('Invalid dimensions', null, 400);
        }

        if (!extension_loaded('gd')) {
            Anon::error('GD extension not available', null, 500);
        }

        $raw = @file_get_contents($filePath);
        if ($raw === false) {
            Anon::error('Failed to read source image', null, 500);
        }

        $src = @imagecreatefromstring($raw);
        if (!$src) {
            Anon::error('Invalid image file', null, 500);
        }
        ...
        if (!$outputFunction) {
            imagedestroy($src);
            Anon::error('Unsupported output format', null, 400);
        }
        ...
        switch ($targetFormat) {
            ...
            case 'avif':
                if (function_exists('imageavif')) {
                    ...
                    imagedestroy($src);
                    Anon::error('AVIF support not available', null, 500);
                }
                break;
        }
    ...
    public static function getAttachmentList(int $page = 1, int $pageSize = 20, ?string $mimeType = null, string $sort = 'new'): array
    {
        try {
            ...
            Debug::error('Failed to get attachment list', ['error' => $e->getMessage()]);
            return ['list' => [], 'total' => 0];
        }
    }
    ...
    {
        $data = array_merge($_GET, RequestHelper::getInput());
    ...
    public static function handleUpload(): void
    {
        ...
        if (empty($ext)) {
            ResponseHelper::error('文件必须包含扩展名', null, 400);
        }
        $realMimeType = self::detectMimeTypeFromFile($file['tmp_name']);
        if (empty($realMimeType)) {
            ResponseHelper::error('无法识别文件类型', null, 400);
        }
        if (!self::isExtensionMatchMimeType($ext, $realMimeType)) {
            ResponseHelper::error("文件扩展名与 MIME 类型不匹配", null, 400);
        }
        $fileType = self::getFileTypeByMime($realMimeType);
        $built = self::buildRandomFilename($ext);
        $uploadDir = Main::APP_DIR . 'Upload/' . $fileType . '/';
        if (!is_dir($uploadDir)) mkdir($uploadDir, 0755, true);
        if (!move_uploaded_file($file['tmp_name'], $uploadDir . $built['filename'])) {
            ResponseHelper::error('保存文件失败', null, 500);
        }
        $db = Database::getInstance();
        $id = $db->db('attachments')->insert(['uid' => RequestHelper::getUserId(), 'filename' => $built['filename'], 'original_name' => $file['name'], 'file_size' => $file['size']]);
        if (!$id) {
            ResponseHelper::error('保存记录失败', null, 500);
        }
        $attachment = self::getAttachment($id);
        $attachment['url'] = '/anon/attachment/' . $fileType . '/' . $built['base'];
        $attachment['mime_type'] = $realMimeType;
        ResponseHelper::success($attachment, '上传成功');
    }
    ...
    public static function handleDelete(): void
    {
        ...
        if ($id <= 0) {
            ResponseHelper::error('附件 ID 无效', null, 400);
        }
        $attachment = self::getAttachment($id);
        if (!$attachment) {
            ResponseHelper::error('附件不存在', null, 404);
        }
        self::delete($id);
