...
use Anon\Modules\System\Plugin\Base as PluginBase;
...
class Plugin
{
    ...
    public static function init(): void
    {
        ...
        if (!self::isEnabled()) {
            Debug::info('Plugin system is disabled');
            self::$initialized = true;
            return;
        }

        Hook::do_action('plugin_system_before_init');
        ...
        Hook::do_action('plugin_system_after_init', self::$loadedPlugins);
    }
    ...
    {
        $appMode = defined('ANON_APP_MODE') ? ANON_APP_MODE : Env::get('app.mode', 'api');
        ...
        if (Env::isInitialized()) {
            return Env::get('app.plugins.enabled', true);
        }
    ...
    private static function scanAndLoadPlugins(): void
    {
        ...
        if (!is_dir($pluginDir)) {
            Debug::warn("Plugin directory not found: {$pluginDir}");
            return;
        }

        Hook::do_action('plugin_before_scan');
        ...
        if ($cachedPlugins !== null) {
            foreach ($cachedPlugins as $pluginData) {
                ...
                Debug::info("Loading plugin (cached): {$pluginData['meta']['name']}", [
                    'slug' => $pluginSlug,
            ...
            if (defined('ANON_DEBUG') && ANON_DEBUG) {
                ...
                Debug::debug("Scanning plugin directory", [
                    'path' => $pluginDir,
            ...
            foreach ($dirs as $dir) {
                ...
                if (!$meta) {
                    Debug::warn("Failed to read plugin meta", ['plugin' => $dir, 'path' => $pluginPath]);
                    continue;
                }
                ...
                Debug::info("Loading plugin: {$meta['name']}", [
                    'slug' => $pluginSlug,
        ...
        Hook::do_action('plugin_after_scan', self::$loadedPlugins);
    }
    ...
    private static function loadScanCache(string $pluginDir): ?array
    {
        ...
        try {
            $cached = Cache::get('plugin_scan_list');
    ...
    private static function saveScanCache(string $pluginDir, array $plugins): void
    {
        ...
        try {
            ...
            Cache::set('plugin_scan_list', $cacheData, 3600);
        } catch (Throwable $e) {
            // 忽略
        }
    }
    ...
    public static function clearScanCache(): void
    {
        ...
        Debug::info('Plugin scan cache cleared');
    }
    ...
    public static function readPluginMetaFromPackageJson(string $pluginPath): ?array
    {
        ...
        try {
            ...
            if (isset($data['anon']) && is_array($data['anon'])) {
                ...
                if (isset($data['anon']['class'])) {
                    $c = trim((string) $data['anon']['class']);
                    if ($c !== '') {
                        $meta['class'] = $c;
                    }
                }
    ...
    public static function readPluginMeta(string $pluginFile): ?array
    {
        try {
            ...
            if (empty($meta['name'])) {
                Debug::warn("Plugin meta missing 'name' field in {$pluginFile}");
                return null;
            }

            return $meta;
        } catch (Throwable $e) {
            Debug::error("Error reading plugin meta from {$pluginFile}: " . $e->getMessage());
            return null;
        }
    }
    ...
    private static function convertPhpArrayToJson(string $phpArrayStr): ?string
    {
        ...
        if (preg_match($unsafePattern, $phpArrayStr)) {
            Debug::debug("Plugin meta contains unsafe code", ['content' => substr($phpArrayStr, 0, 100)]);
            return null;
        }

        if (!preg_match('/\[[\s\S]*\]/', $phpArrayStr)) {
            Debug::debug("Plugin meta is not a valid array structure", ['content' => substr($phpArrayStr, 0, 200)]);
            return null;
        }
        ...
        Debug::debug("Plugin meta JSON conversion failed", [
    ...
    private static function getActivePlugins(): array
    {
        ...
        if ($appMode === 'cms') {
            $active = CmsOptions::get('plugins:active', []);
            if (is_string($active)) {
        ...
        if (Env::isInitialized()) {
            $active = Env::get('app.plugins.active', []);
            if (empty($active)) {
                return [];
            }
            return array_map('strtolower', $active);
    ...
    {
        Hook::do_action('plugin_before_load', $pluginSlug, $meta);

        try {
            if (isset(self::$loadedPlugins[$pluginSlug])) {
                Debug::debug("Plugin already loaded, skipping", ['plugin' => $pluginSlug]);
                return;
            }

            require_once $pluginFile;

            $className = isset($meta['class']) && is_string($meta['class']) && trim($meta['class']) !== ''
                ? trim($meta['class'])
                : self::getPluginClassName($dirName);
            $pluginInstance = null;
            ...
            if ($actualClassName && class_exists($actualClassName)) {
                ...
                try {
                    if (is_subclass_of($className, PluginBase::class)) {
                        ...
                        Debug::debug("Plugin initialized (instance)", ['class' => $className]);
                    } elseif (method_exists($className, 'init')) {
                        $className::init();
                        Debug::debug("Plugin initialized", ['class' => $className]);
                        $pluginInstance = $className;
                    } else {
                        Debug::warn("Plugin class has no init() method", ['class' => $className]);
                        $pluginInstance = $className;
                    }
                } catch (Throwable $e) {
                    Debug::error("Plugin initialization failed", [
                        'plugin' => $pluginSlug,
                        'class' => $className,
                        'error' => $e->getMessage(),
                        'file' => $e->getFile(),
                ...
                Debug::warn("Plugin class not found", [
                    'plugin' => $pluginSlug,
                    'expected' => $className,
                    'dir' => $dirName
                ]);
            ...
            // 自动注册插件的中间件
            if (class_exists(Extension::class) && method_exists($className, 'registerMiddleware')) {
                try {
                    $className::registerMiddleware();
                    Debug::debug("Plugin middleware registered", ['plugin' => $pluginSlug]);
                } catch (Throwable $e) {
                    Debug::error("Failed to register plugin middleware", [
                        'plugin' => $pluginSlug,
                        'error' => $e->getMessage()
                    ]);
                }
            ...
            Hook::do_action('plugin_after_load', $pluginSlug, $meta);
        } catch (Throwable $e) {
            Debug::error("Failed to load plugin", [
                'plugin' => $pluginSlug,
                'error' => $e->getMessage(),
                'file' => $e->getFile(),
                'line' => $e->getLine()
            ...
            Hook::do_action('plugin_load_error', $pluginSlug, $e);
        }
    }
    ...
    private static function getPluginClassName(string $dirName): string
    {
        ...
        $className = 'Anon\\Modules\\System\\Plugin\\';
    ...
    public static function getPluginClassNameFromSlug(string $slug): ?string
    {
        ...
        $pluginPath = self::PLUGIN_DIR . $resolvedDir;
        $meta = self::readPluginMetaForDir($pluginPath);
        if (is_array($meta) && isset($meta['class']) && is_string($meta['class']) && trim($meta['class']) !== '') {
            return trim($meta['class']);
        }

        return self::getPluginClassName($resolvedDir);
    }
        ...
        if ($pluginInfo && isset($pluginInfo['class']) && class_exists($pluginInfo['class'])) {
            ...
            if (method_exists($className, 'activate')) {
                try {
                    ...
                    Debug::error("Failed to activate plugin {$pluginSlug}: " . $e->getMessage());
                    return false;
                }
            }
        }
        ...
        if ($pluginInfo && isset($pluginInfo['class']) && class_exists($pluginInfo['class'])) {
            ...
            if (method_exists($className, 'deactivate')) {
                try {
                    ...
                    Debug::error("Failed to deactivate plugin {$pluginSlug}: " . $e->getMessage());
                    return false;
                }
            }
        }
    ...
    public static function reloadPlugin(string $pluginSlug): bool
    {
        ...
        if ($resolvedDir === null) {
            if (defined('ANON_DEBUG') && ANON_DEBUG) {
                Debug::warn("Plugin directory not found", ['slug' => $pluginSlug]);
            }
            return false;
        }
        ...
        if ($pluginFile === null) {
            if (defined('ANON_DEBUG') && ANON_DEBUG) {
                Debug::warn("Plugin main file not found", ['slug' => $pluginSlug, 'path' => $pluginPath]);
            }
            return false;
        }
        ...
        if (!$meta || empty($meta['name'])) {
            if (defined('ANON_DEBUG') && ANON_DEBUG) {
                Debug::warn("Failed to read plugin meta", ['slug' => $pluginSlug]);
            }
            return false;
        }
        ...
        // 按模式匹配检查插件是否应加载
        if (!self::shouldLoadPlugin($meta)) {
            Debug::debug("Plugin mode mismatch, skipping", ['slug' => $pluginSlug, 'mode' => $meta['mode'] ?? 'api']);
            return false;
        }

        // 检查插件是否已激活
        if (!self::isPluginActive($pluginSlug)) {
            Debug::debug("Plugin not active, skipping", ['slug' => $pluginSlug]);
            return false;
        }
    ...
    {
        return defined('ANON_APP_MODE') ? ANON_APP_MODE : Env::get('app.mode', 'api');
    }
