...
class AccessLog
{
    ...
    {
        return CmsOptions::get('access_log_enabled', true);
    }
    ...
    public static function log(array $options = []): void
    {
        ...
        try {
            ...
            Debug::error("访问日志记录失败", ['message' => $e->getMessage()]);
        }
    }
    ...
    private static function shouldLog(): bool
    {
        ...
        // 通过钩子允许插件或配置动态决定是否记录
        $shouldLog = Hook::apply_filters('access_log_should_log', null, [
            'path' => $path,
            'uri' => $requestUri
        ]);
        ...
        // 检查是否记录 API 请求
        $logApi = CmsOptions::get('access_log_api', false);
        if (!$logApi && (strpos($path, '/api/') === 0 || strpos($path, '/anon/') === 0)) {
            return false;
        }

        // 检查是否记录静态资源
        $logStatic = CmsOptions::get('access_log_static', false);
    ...
    private static function buildQueryWithOptions($db, array $options): QueryBuilder
    {
    ...
    public static function cleanOldLogs(int $days = 90): int
    {
        try {
            ...
            Debug::error("清理访问日志失败", ['message' => $e->getMessage()]);
            return 0;
        }
    }
}
