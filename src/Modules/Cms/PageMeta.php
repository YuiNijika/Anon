...
class PageMeta
{
    ...
    public static function get(array $overrides = []): array
    {
        ...
        if (!empty($GLOBALS['_anon_rendering_error_page'])) {
            $siteTitle = trim((string) CmsOptions::get('title', ''));
        ...
        if (!empty($overrides) && (isset($overrides['title']) || isset($overrides['description']) || isset($overrides['robots']))) {
            ...
            if ($pageType === 'error') {
                ...
                $meta = Hook::apply_filters('cms_page_meta_error', $meta, $errorInfo);
            } elseif (defined('\Anon\PageMeta')) {
                $meta = is_array(\Anon\PageMeta) ? \Anon\PageMeta : [];
        ...
        $meta = array_merge($defaults, $meta);
        $meta['canonical'] = self::getCanonical();
        $meta['og'] = ['title' => $meta['title'] ?? Common::NAME, 'description' => $meta['description'] ?? '', 'type' => 'website', 'url' => $meta['canonical']];
        $meta['twitter'] = ['card' => 'summary', 'title' => $meta['title'] ?? Common::NAME, 'description' => $meta['description'] ?? ''];
        return $meta;
    }
    ...
    public static function getError(array $overrides = []): array
    {
        ...
        if (defined('\Anon\PageMeta') && is_array(\Anon\PageMeta)) {
            $metaTitle = \Anon\PageMeta['title'] ?? null;
            $metaDescription = \Anon\PageMeta['description'] ?? null;
        }
        return [
            'code' => $statusCode,
    ...
    {
        if (defined('\Anon\PageMeta') && is_array(\Anon\PageMeta) && !empty(\Anon\PageMeta['keywords'])) {
            $k = \Anon\PageMeta['keywords'];
    ...
    private static function getIndexMeta(): array
    {
        ...
        $siteSubtitle = CmsOptions::get('subtitle', '');
        $siteDesc = CmsOptions::get('description', '');
        $siteKw = CmsOptions::get('keywords', '');
    ...
    private static function getUserData(): ?array
    {
        ...
        if ($uid !== null && $uid > 0) {
            return Cms::getUserByUid((int) $uid);
        }
        if (is_string($name) && $name !== '') {
            return Cms::getUserByName($name);
        }
        return null;
    }
