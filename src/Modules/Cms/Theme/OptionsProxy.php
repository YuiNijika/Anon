...
class OptionsProxy
{
    ...
    public function get(string $name, $default = null, $outputOrPriority = true, ?string $priority = null)
    {
        ...
        if (Debug::isEnabled()) {
            Debug::debug('[OptionsProxy] get() 参数解析', [
                'option_name' => $name,
        ...
        foreach ($order as $source) {
            if ($source === 'plugin' && $this->pluginSlug !== null && $this->pluginSlug !== '') {
                    $all = Plugin::getPluginOptions($this->pluginSlug);
            ...
            if ($source === 'system') {
                $value = CmsOptions::get($name, $default);
                break;
            }
        }

        if (is_object($value) && $value instanceof OptionsProxy) {
            if (Debug::isEnabled()) {
                Debug::error('[OptionsProxy] get() 返回了代理对象', [
                    'option_name' => $name,
                    'default' => $default,
                    'outputOrPriority' => $outputOrPriority,
                    'priority' => $priority,
    ...
    {
        return CmsOptions::set($name, $value);
    }
}
