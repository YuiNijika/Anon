...
class Theme
{
    ...
    private static function getMimeTypes(): array
    {
        if (self::$mimeTypesCache === null) {
            ...
            self::$mimeTypesCache = Hook::apply_filters('cms_theme_mime_types', $defaultMimeTypes);
        }
    ...
    private static function getTypeDirs(): array
    {
        if (self::$typeDirsCache === null) {
            ...
            self::$typeDirsCache = Hook::apply_filters('cms_theme_type_dirs', $defaultTypeDirs);
        }
    ...
    public static function init(?string $themeName = null): void
    {
        try {
            ...
            if ($themeName === null) {
                $themeName = Env::get('app.cms.theme', 'Default');
            }
    ...
    private static function registerStaticResources(): void
    {
        try {
            ...
            StaticResource::registerThemeRoute($themeName, 'assets');
            StaticResource::registerThemeRoute($themeName, 'css');
            StaticResource::registerThemeRoute($themeName, 'js');
            StaticResource::registerThemeRoute($themeName, 'images');
            StaticResource::registerThemeRoute($themeName, 'img');
            StaticResource::registerThemeRoute($themeName, 'fonts');
            
            Debug::info("Registered static resources for theme: {$themeName}");
        } catch (Exception $e) {
            Debug::error('Failed to register theme static resources', [
    ...
    private static function loadThemeCode(): void
    {
        try {
            ...
            if (Cms::fileExists($codeFile)) {
                $themeName = self::getCurrentTheme();
                $codeContext = new ThemeHelper($themeName);
    ...
    public static function getSchemaFromSetupFile(string $themeName): array
    {
        ...
        $helper = new SetupHelper($themeName);
    ...
    private static function loadThemeSetupFile(string $themeDir, ?string $themeName = null): void
    {
        ...
        if ($themeDirResolved === false) {
            Debug::warn('[Theme] loadThemeSetupFile: themeDir not found', ['themeDir' => $themeDir]);
            return;
        }
        $setupFile = $themeDirResolved . DIRECTORY_SEPARATOR . 'app' . DIRECTORY_SEPARATOR . 'setup.php';
        if (!file_exists($setupFile)) {
            Debug::warn('[Theme] loadThemeSetupFile: setup.php not found', ['setupFile' => $setupFile]);
            return;
        }
        if (!defined('ANON_ALLOWED_ACCESS')) {
            define('ANON_ALLOWED_ACCESS', true);
        ...
        $helper = new SetupHelper($name);
        $schema = $helper->loadSetupFile($setupFile);

        if (!is_array($schema)) {
            Debug::warn('[Theme] loadThemeSetupFile: setup.php did not return array', ['got' => gettype($schema)]);
            return;
        }
        Options::registerFromSchema($schema, strtolower($name));
        CmsOptions::clearCache();
        Debug::info('[Theme] loadThemeSetupFile: registered schema for theme', ['theme' => $name, 'keys' => count($schema)]);
    }
    ...
    private static function findThemeDirectory(string $themeName): ?string
    {
        ...
        $found = Cms::findDirectoryCaseInsensitive($themesBase . DIRECTORY_SEPARATOR, $themeName);
        if ($found === null) {
            return null;
        }
    ...
    public static function ensureThemeOptionsLoaded(string $themeName): void
    {
        ...
        $val = CmsOptions::get("theme:{$keyName}", $missing);
    ...
    public static function render(string $templateName, array $data = []): void
    {
        try {
            ...
            if ($templatePath === null) {
                $pageType = Cms::getPageType($templateName);
            ...
            $view = new View($data, $templateName);
    ...
    public static function findTemplate(string $templateName): ?string
    {
        ...
        $templatePath = Cms::findFileCaseInsensitive($themeDir, $templateName);
    ...
    public static function assets(string $path, $forceNoCacheOrType = null, array $attributes = []): string
    {
        ...
        $themeDir = self::getThemeDir();
        $assetsDir = Cms::findDirectoryCaseInsensitive($themeDir, 'assets');
        ...
        try {
            ...
            if ($componentsDir === null) {
                if (defined('ANON_DEBUG') && ANON_DEBUG) {
                    Debug::warn('[Anon Theme components] 静态调用: 组件目录未找到', ['themeDir' => $themeDir, 'lookup' => 'app/components']);
                }
            ...
            foreach ($pathParts as $part) {
                ...
                if ($foundDir === null) {
                    Debug::warn('[Anon Theme components] 静态调用: 组件子目录未找到', ['componentPath' => $componentPath, 'part' => $part, 'componentDir' => $componentDir]);
            ...
            Debug::warn('[Anon Theme components] 静态调用: 组件文件未找到', ['componentPath' => $componentPath, 'componentName' => $componentName, 'componentDir' => $componentDir]);
    ...
    {
        $siteTitle = CmsOptions::get('title', '');
    ...
    public static function head(array $options = []): void
    {
        ...
        $author = $options['author'] ?? CmsOptions::get('author', '');
    ...
    {
        $charset = CmsOptions::get('charset', 'UTF-8');
        echo '<meta charset="' . htmlspecialchars((string) $charset) . '">' . "\n";
        echo '<meta name="viewport" content="width=device-width, initial-scale=1.0">' . "\n";
        try {
            $seo = PageMeta::getSeo($overrides);
            ...
            self::meta($seo);
            echo '<script src="/anon/static/vue?ver=' . \Anon\Modules\Common::VERSION . '"></script>' . "\n";
            echo '<script src="/anon/static/comments?ver=' . \Anon\Modules\Common::VERSION . '"></script>' . "\n";
            Hook::do_action('theme_head');
        } catch (Error $e) {
            self::handleFatalError($e);
        } catch (Throwable $e) {
            if (self::isFatalError($e)) {
    ...
    public static function getAllThemes(): array
    {
        ...
        $themesDir = \Anon\Main::APP_DIR . 'Theme/';
        $themes = [];

        if (!Cms::isDir($themesDir)) {
            self::$allThemesCache = $themes;
            return $themes;
        }

        $items = Cms::scanDirectory($themesDir);
        if ($items === null) {
            self::$allThemesCache = $themes;
            return $themes;
        }
        ...
        foreach ($items as $item) {
            ...
            $themeInfo = [];
            if ($infoFile && Cms::fileExists($infoFile)) {
            ...
            if (!empty($themeInfo['screenshot'])) {
                ...
                if (Cms::fileExists($screenshotFile)) {
    ...
    public static function info(?string $key = null)
    {
        ...
        if (!isset(self::$themeInfoCache[$cacheKey])) {
            ...
            $packageFile = Cms::findFileCaseInsensitive($themeDir, 'package', ['json']);
            if ($packageFile !== null && Cms::fileExists($packageFile)) {
            ...
            $headerFile = Cms::findFileCaseInsensitive($themeDir, 'index', ['php']);
            if ($headerFile === null) {
                $headerFile = Cms::findFileCaseInsensitive($themeDir, 'theme', ['php']);
            }
            
            if ($headerFile !== null && Cms::fileExists($headerFile)) {
    ...
    {
        try {
            $apiPrefix = CmsOptions::get('apiPrefix', '/api');
            $apiPrefix = rtrim($apiPrefix, '/');

            $userLoggedIn = Check::isLoggedIn();
            $userInfo = [];
            if ($userLoggedIn) {
            ...
            if (self::is('index')) {
                ...
                $currentObject = Cms::getPostIfExists();
                if (!$currentObject) {
                    $currentObject = Cms::getPageIfExists();
                }
    ...
    public static function registerSetting(string $key, array $args = []): void
    {
        ...
        $settings = CmsOptions::get($optionName, []);
        ...
        $registeredSettings = CmsOptions::get("theme:{$themeName}:settings", []);
        if (!is_array($registeredSettings)) {
            $registeredSettings = [];
        }
        $registeredSettings[$key] = $args;
        CmsOptions::set("theme:{$themeName}:settings", $registeredSettings);

        CmsOptions::set($optionName, $settings);
    }
    ...
    public static function setSetting(string $key, $value): bool
    {
        ...
        $settings[$key] = $value;
        return CmsOptions::set($optionName, $settings);
    }
