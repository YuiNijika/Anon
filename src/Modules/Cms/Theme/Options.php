...
class Options
{
    ...
    public static function register(string $key, array $args = [], ?string $themeName = null): void
    {
        ...
        $settings = CmsOptions::get($optionKey, []);
        if (!is_array($settings)) {
            $settings = [];
        }

        $schema = CmsOptions::get($schemaKey, []);
        if (!is_array($schema)) {
            $schema = [];
        }
        ...
        CmsOptions::set($schemaKey, $schema);
        CmsOptions::set($optionKey, $settings);
    }
    ...
    {
        $settings = CmsOptions::get(self::optionKey($themeName), []);
        if (!is_array($settings)) {
            return $default;
        }
    ...
    {
        $schema = CmsOptions::get(self::schemaKey($themeName), []);
