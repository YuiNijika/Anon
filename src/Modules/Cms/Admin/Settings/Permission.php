...
class Permission
{
    ...
    public static function get()
    {
        try {
            ...
            $settings = [
                'allow_register' => $toBool(Options::get('allow_register', '0')),
                'access_log_enabled' => $toBool(Options::get('access_log_enabled', '1')),
                'api_prefix' => Options::get('apiPrefix', '/api'),
                'api_enabled' => $toBool(Options::get('api_enabled', '0')),
                'restful_api_token_required' => $toBool(Options::get('restful_api_token_required', '1')),
            ];
            
            ResponseHelper::success($settings, '获取权限设置成功');
        } catch (Exception $e) {
    ...
    public static function save()
    {
        try {
            ...
            if (empty($apiPrefix) || $apiPrefix[0] !== '/') {
                ResponseHelper::error('API 前缀必须以 / 开头', null, 400);
                return;
            }

            Options::set('allow_register', $allowRegister);
            Options::set('apiPrefix', $apiPrefix);
            Options::set('api_enabled', $apiEnabled);
            Options::set('access_log_enabled', $accessLogEnabled);
            Options::set('restful_api_token_required', $restfulApiTokenRequired);
            
            /**
             * 清除选项缓存和 API 前缀缓存，确保设置立即生效
             */
            Options::clearCache();
            ApiPrefix::clearCache();
