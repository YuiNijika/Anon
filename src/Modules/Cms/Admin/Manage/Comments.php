...
class Comments
{
    ...
    public static function getList()
    {
        try {
            ...
            foreach ($rows as &$row) {
                ...
                } elseif ((!isset($row['avatar']) || $row['avatar'] === '') && !empty($row['email'])) {
                    $row['avatar'] = $db->getAvatarByEmail($row['email']);
                }
    ...
    public static function update()
    {
        try {
            ...
            if ($id <= 0) {
                ResponseHelper::error('参数无效', null, 400);
            }
            $db = Database::getInstance();
            $exists = $db->db('comments')->where('id', $id)->first();
            if (!$exists) {
                ResponseHelper::error('评论不存在', null, 404);
            }
            $update = [];
            ...
            if (empty($update)) {
                ResponseHelper::error('请提供 status 或 content', null, 400);
            }
            $db->db('comments')->where('id', $id)->update($update);
            ResponseHelper::success(null, '更新成功');
        } catch (Exception $e) {
            ResponseHelper::error($e->getMessage(), null, 500);
        }
    }
    ...
    public static function delete()
    {
        try {
            ...
            if ($affected > 0) {
                ...
                ResponseHelper::error('评论不存在或已删除', null, 404);
            }
        } catch (Exception $e) {
            ResponseHelper::error($e->getMessage(), null, 500);
        }
