...
class Cms
{
    ...
    private static function outputPageLoadTimeScript(): void
    {
        ...
        $queryCount = Database::getQueryCount() ?? null;
        $console = 'console.log("页面加载耗时：' . $loadTime . 'ms | SQL 查询：' . ($queryCount ?? 0) . '次");';
        echo $console;
    }
        ...
        if (!isset(self::$viewedPosts[$id])) {
            ...
            register_shutdown_function(function () use ($id, $currentViews) {
                try {
                    ...
                    Debug::error("异步增加文章浏览量失败", ['message' => $e->getMessage()]);
                }
            });
        }
        ...
        if (!isset(self::$viewedPosts[$page['id']])) {
            ...
            register_shutdown_function(function () use ($page, $currentViews) {
                try {
                    ...
                    Debug::error("异步增加页面浏览量失败", ['message' => $e->getMessage()]);
                }
            });
        }
    ...
    {
        $base = Options::get('site_url', '');
        if ($base === '' || $base === null) {
            $base = Options::get('home', '');
        }
    ...
    private static function enrichCommentsForTheme(array $rows, $db): array
    {
        ...
        foreach ($rows as &$r) {
            if (!empty($r['type']) && $r['type'] === 'user' && isset($r['uid']) && isset($userMap[(int) $r['uid']])) {
                ...
                $r['url'] = $r['url'] ?? null;
            } elseif ((!isset($r['avatar']) || $r['avatar'] === '') && !empty($r['email'])) {
                $r['avatar'] = $db->getAvatarByEmail($r['email']);
            }
        }
        unset($r);
        $byId = [];
    ...
    public static function handleCommentsGet(): void
    {
        ...
        ResponseHelper::success($data, '获取成功');
    }
    ...
    public static function handleCommentSubmit(): void
    {
        if ($_SERVER['REQUEST_METHOD'] !== 'POST') {
            ResponseHelper::error('仅支持 POST', null, 405);
            return;
        }
        ...
        if ($postId <= 0 || $content === '') {
            if ($isAjax) {
                ResponseHelper::error($uid ? '请填写评论内容' : '请填写名称、邮箱和评论内容');
                return;
            }
        ...
        if ($commentType === 'guest' && ($name === '' || $email === '')) {
            if ($isAjax) {
                ResponseHelper::error('请填写名称、邮箱和评论内容');
                return;
            }
            $redirect('comment=error');
        }
        ...
        // 验证码验证
        if ($commentType === 'guest' && Env::get('app.base.captcha.enabled', false)) {
            // 确保验证码模块已加载
            if (Captcha::isEnabled()) {
                $captchaCode = trim((string) ($_POST['captcha'] ?? ''));
                if ($captchaCode === '') {
                    if ($isAjax) {
                        ResponseHelper::error('请输入验证码');
                        return;
                    }
                    $redirect('comment=captcha_required');
                }
                if (!Captcha::verify($captchaCode)) {
                    if ($isAjax) {
                        ResponseHelper::error('验证码错误，请重新输入');
                        return;
                    }
        ...
        if (!$post || ($post['type'] ?? '') !== 'post') {
            if ($isAjax) {
                ResponseHelper::error('文章不存在或无法评论');
                return;
            }
            $redirect('comment=error');
        }
        ...
        try {
            ...
            Database::getInstance()->db('comments')->insert($insert);
        } catch (\Throwable $e) {
            if ($isAjax) {
                ResponseHelper::error('提交失败，请稍后重试');
                return;
            }
            $redirect('comment=error');
        }
        if ($isAjax) {
            $msg = ($initialStatus === 'approved') ? '评论发表成功' : '评论已提交，待审核通过后会显示。';
            ResponseHelper::success(null, $msg);
            return;
        }
        $redirect(($initialStatus === 'approved') ? 'comment=ok' : 'comment=pending');
    }
    ...
    public static function handleCommentsRequest(): void
    {
        ...
        ResponseHelper::error('仅支持 GET 或 POST', null, 405);
    }
}
