 ...
 */
class Process implements ServerInterface
{
    /**
     * @var array 进程列表
     */
    ...
    {
        if (!class_exists('SwooleProcess')) {
            throw new Exception("未安装 Swoole 扩展。");
        }

        $process = new SwooleProcess($callback, $redirectStdinStdout, $pipeType, $enableCoroutine);
    ...
    public function start(array $options = []): void
    {
        ...
        if (php_sapi_name() === 'cli' && class_exists('SwooleProcess')) {
    ...
    public function stop(): void
    {
        foreach ($this->processes as $process) {
            // 发送 SIGTERM 信号
            SwooleProcess::kill($process->pid, SIGTERM);
        }
    }
