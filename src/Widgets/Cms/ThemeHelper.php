...
class ThemeHelper
{
    ...
    /**
     * @var OptionsProxy|null 选项代理缓存
     */
    private $optionsProxy;

    /**
     * 构造主题辅助对象
     * @param string $themeName 主题名，通常由 Theme::getCurrentTheme() 传入
     */
    ...
    {
        return Options::get($key, $default, $this->themeName);
    }
    ...
    public function options(?string $name = null, $default = null, $outputOrPriority = false, ?string $priority = null)
    {
        if ($this->optionsProxy === null) {
            $this->optionsProxy = new OptionsProxy('theme', null, $this->themeName);
        }
        ...
        try {
            ...
            if (class_exists(Debug::class) && Debug::isEnabled()) {
                Debug::debug('[Theme Helper] options() 调用结果', [
                    'theme' => $this->name(),
                    'option_name' => $name,
                    'default' => $default,
            ...
            if (is_object($result)) {
                if ($result instanceof OptionsProxy) {
                    if (class_exists(Debug::class) && Debug::isEnabled()) {
                        Debug::error('[Theme Helper] options() 返回了代理对象', [
                            'theme' => $this->name(),
                            'option_name' => $name,
                ...
                if (method_exists($result, '__toString')) {
                    ...
                    if (class_exists(Debug::class) && Debug::isEnabled()) {
                        Debug::warn('[Theme Helper] options() 返回了对象，已转换为字符串', [
                            'theme' => $this->name(),
                            'option_name' => $name,
                            'result_type' => get_class($result),
                ...
                if (class_exists(Debug::class) && Debug::isEnabled()) {
                    Debug::error('[Theme Helper] options() 返回了无法转换的对象', [
                        'theme' => $this->name(),
                        'option_name' => $name,
                        'default' => $default,
                        'result_type' => get_class($result),
            ...
            if (class_exists(Debug::class) && Debug::isEnabled()) {
                Debug::error('[Theme Helper] options() 发生异常', [
                    'theme' => $this->name(),
                    'option_name' => $name,
                    'default' => $default,
                    'outputOrPriority' => $outputOrPriority,
    ...
    {
        return Theme::getViteDevServerUrl() !== null;
    }
    ...
    {
        return Theme::getViteDevServerUrl();
    }
    ...
    {
        Theme::partial($partialName, $data);
    }
    ...
    {
        Theme::components($componentPath, $data);
    }
    ...
    {
        Theme::title($title, $separator, $reverse);
    }
    ...
    {
        return Theme::getCurrentTheme() === $this->themeName;
    }
    ...
    {
        Theme::stylesheet($styles, $attributes);
    }
    ...
    {
        Theme::script($scripts, $attributes);
    }
    ...
    {
        return Theme::escape($text, $flags);
    }
    ...
    public function framework(string $type): string
    {
        if ($type === 'name') {
            ...
            $frame = Common::VERSION;
        } elseif ($type === 'author') {
            $frame = Common::AUTHOR;
        } elseif ($type === 'author_url') {
            $frame = Common::AUTHOR_URL;
        } elseif ($type === 'license') {
            $frame = Common::LICENSE;
        } elseif ($type === 'license_text') {
            $frame = Common::LICENSE_TEXT();
        } elseif ($type === 'github') {
            $frame = Common::GITHUB;
        } else {
            $frame = Common::NAME;
    ...
    {
        if ($type === 'name') {
            $server = Common::server('name');
        } elseif ($type === 'version') {
            $server = Common::server('version');
        } elseif ($type === 'os') {
            $server = Common::server('os');
        } elseif ($type === 'os_version') {
            $server = Common::server('os_version');
        } elseif ($type === 'ip') {
            $server = Common::server('ip');
        } elseif ($type === 'port') {
            $server = (string) Common::server('port');
        } elseif ($type === 'url') {
            $server = Common::server('url');
        } elseif ($type === 'protocol') {
            $server = Common::server('protocol');
        } elseif ($type === 'domain') {
            $server = Common::server('domain');
        } elseif ($type === 'isHttps') {
            $server = Common::server('is_https') ? 'true' : 'false';
        } elseif ($type === 'php') {
            $server = Common::server('php');
        } else {
            $server = Common::server('name');
        }
