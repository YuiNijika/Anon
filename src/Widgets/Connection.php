...
class Connection
{
    ...
    public function query(string $sql)
    {
        ...
        if (
            ...
            Debug::warn("警告：检测到可疑的 SQL 查询模式", ['sql_preview' => substr($sql, 0, 100)]);
        }

        $result = $this->conn->query($sql);
        if (!$result) {
            $errorMsg = self::sanitizeError($this->conn->error);
            Debug::error("SQL 查询错误", ['error' => $errorMsg]);
    ...
    public function prepare(string $sql, array $params = []): mysqli_stmt
    {
        ...
        if (!$stmt) {
            $errorMsg = self::sanitizeError($this->conn->error);
            Debug::error("SQL 预处理错误", ['error' => $errorMsg]);
        ...
        if (!empty($params)) {
            ...
            if (!$stmt->bind_param($types, ...$bindParams)) {
                $errorMsg = self::sanitizeError($this->conn->error);
                Debug::error("SQL 参数绑定错误", ['error' => $errorMsg]);
                $stmt->close();
    ...
    public function db(string $table): QueryBuilder
    {
        ...
        return new QueryBuilder($this, $this->getTableName($table));
    }
}
