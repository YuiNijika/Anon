...
class XssFilterMiddleware implements MiddlewareInterface
{
    ...
    public function handle($request, callable $next)
    {
        ...
        if (!empty($_POST)) {
            $_POST = Security::filterInput($_POST, $filterOptions);
        }
        
        if (!empty($_GET)) {
            $_GET = Security::filterInput($_GET, $filterOptions);
        }
        
        // 过滤JSON输入
        $contentType = $_SERVER['CONTENT_TYPE'] ?? '';
        if (stripos($contentType, 'application/json') !== false) {
            $jsonInput = RequestHelper::getInput();
            if (!empty($jsonInput) && is_array($jsonInput)) {
                $filteredInput = Security::filterInput($jsonInput, $filterOptions);
                RequestHelper::setFilteredInput($filteredInput);
            }
        }
