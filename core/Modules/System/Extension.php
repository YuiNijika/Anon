...
class Anon_System_Extension
{
    ...
    public static function register(string $name, $handler, int $priority = 10, array $options = []): void
    {
        ...
        Anon_Debug::debug("Extension registered", ['name' => $name, 'priority' => $priority]);
    }
    ...
    public static function unregister(string $name): void
    {
        if (isset(self::$middlewares[$name])) {
            ...
            Anon_Debug::debug("Extension unregistered", ['name' => $name]);
        }
    }
    ...
    private static function callHandler($handler, $data)
    {
        try {
            ...
            Anon_Debug::error("Extension handler not callable", ['handler' => $handler]);
            return $data;
        } catch (Throwable $e) {
            Anon_Debug::error("Extension execution failed", [
    ...
    public static function init(): void
    {
        ...
        Anon_System_Hook::do_action('extension_system_initialized', self::$middlewares);
    }
    ...
    {
        self::$middlewares = [];
        Anon_Debug::info("All extensions cleared");
    }
}
