...
- **路由前缀**：`/anon/cms/admin`  
- **访问示例**：`https://your-domain.com/anon/cms/admin`（具体入口以部署的前端为准，如独立 Admin SPA 的根路径即后台首页）
...
| 功能 | 说明 |
|------|------|
| **设置** | 基本设置（站点名称、描述、关键词、API 前缀等）、主题切换、主题设置项 |
| **主题** | 主题列表、切换主题、主题选项 |
| **插件** | 插件管理（视实现而定） |
...
管理后台前端通过 CMS 管理端点与后端交互，需使用 `/anon/cms/admin` 前缀。认证流程（Token、登录状态检查、用户信息）与通用 [前端框架集成](../client.md#cms-管理后台-api) 一致，仅 baseURL 指向 CMS 管理前缀。详见 [前端框架集成](../client.md)。
...
- 主题/插件上传：支持 ZIP 包，无目录层级时按约定解析（具体以实现为准）。
