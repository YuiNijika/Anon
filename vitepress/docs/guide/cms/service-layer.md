...
Anon_System_Hook::add_action('cms_post_after_create', function($postId, $data) {
    ...
    // 记录日志
    Anon_Debug::info("文章创建: {$postId}", $data);
}, 10, 2);

// 文章删除前检查
Anon_System_Hook::add_action('cms_post_before_delete', function($id, $post) {
    // 检查是否有评论
...
Anon_System_Hook::add_filter('cms_post_list_params', [Hooks::class, 'modifyParams']);
```
...
```php
// 只需要 1 个参数
Anon_System_Hook::add_action('cms_post_after_create', function($postId) {
    // ...
}, 10, 1);
```
...
{
    $cacheKey = "post_{$id}";
    $cached = Anon_System_Cache::get($cacheKey);
    ...
    if ($post) {
        Anon_System_Cache::set($cacheKey, $post, 3600); // 缓存 1 小时
    }
...
{
    // 调用分类服务验证分类
    $category = Anon_Cms_Services_Category::getById($data['category_id']);
    if (!$category) {
        throw new Exception('分类不存在');
    }
