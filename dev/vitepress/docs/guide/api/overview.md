...
本节说明何时选择API模式、特点与目录/配置要点，并引导到路由与API参考，仅适用于API模式。

API模式是Anon Framework的默认模式，面向纯接口开发，所有响应为JSON，无模板渲染，适合对接前端、移动端或作为微服务。
...
- **模式配置**：`app/useEnv.php` 中 `ANON_APP_MODE` 为 `api`
- **路由目录**：`app/Router/`，文件名与目录结构映射为 URL 路径
- **应用配置**：`app/useApp.php` 中可配置 `router`、`token`、`security`、`cache` 等
