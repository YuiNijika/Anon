...
$value = RequestHelper::get('username'); // 不存在返回null

// 获取POST参数
$value = RequestHelper::post('key', 'default');

// 获取GET参数
$value = RequestHelper::getParam('key', 'default');

// 获取必需参数（不存在则返回错误）
$value = RequestHelper::require('username', '用户名不能为空');
```
...
RequestHelper::requireMethod(['POST', 'PUT']);

// 检查请求方法
$method = RequestHelper::method(); // 'GET'、'POST'等
$isPost = RequestHelper::isPost();
$isGet = RequestHelper::isGet();
```
...
ResponseHelper::success(['id' => 1, 'name' => 'test'], '创建成功');

// 分页响应
ResponseHelper::paginated($data, $pagination, '获取数据成功', 200);
...
// 基本错误响应
ResponseHelper::error('操作失败', $data, 400);
...
// 未找到，返回404
ResponseHelper::notFound('资源不存在');

// 方法不允许，返回405
ResponseHelper::methodNotAllowed('GET, POST');

// 服务器错误，返回500
ResponseHelper::serverError('服务器内部错误', $data);
```
...
try {
    ...
    ResponseHelper::handleException($e, '操作时发生错误');
...
```php
// 设置HTTP响应头
Common::Header(200, true, true);              // 200, JSON, CORS
Common::Header404();                          // 404, JSON, CORS
Common::Header(200, false);                  // 200, 非JSON, CORS
Common::Header(200, true, false);            // 200, JSON, 非CORS
...
try {
    ...
    // 返回成功响应
    ResponseHelper::success($result, '创建成功', 201);
    
} catch (Exception $e) {
    ResponseHelper::handleException($e, '创建用户时发生错误');
}
```
