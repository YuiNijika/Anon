...
`Database` 已改为单例模式，确保同一请求生命周期内复用数据库连接，减少连接创建开销。
...
实现原理包括私有构造函数防止外部实例化，`getInstance()` 方法确保全局唯一实例，防止克隆和反序列化，同一请求内多次调用返回同一实例。
...
Hook::add_filter('my_filter', 'my_callback');
...
Hook::add_filter('my_filter', [MyClass::class, 'callback']);

// 不推荐：使用闭包（影响性能）
Hook::add_filter('my_filter', function($data) {
    return $data;
});
```
...
`Env` 内部缓存已加载的配置，首次解析后存入内存，后续调用直接读取缓存。

#### 工作原理

```php
// 首次调用：解析配置并缓存
$value1 = Env::get('app.token.enabled');

// 后续调用：直接从缓存读取
$value2 = Env::get('app.token.enabled');
```
...
// 推荐：使用命名函数或类方法
Hook::add_action('my_action', 'my_function');
Hook::add_filter('my_filter', [MyClass::class, 'method']);
```
...
// 配置会自动缓存，无需特殊处理
$enabled = Env::get('app.token.enabled', false);
```
