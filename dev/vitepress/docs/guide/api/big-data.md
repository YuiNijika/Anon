...
<?php
use Anon\Modules\Database\Database;
use Anon\Modules\Utils\QueryOptimizer;
...
// 批量加载用户的订单（避免 N+1 查询）
$users = QueryOptimizer::eagerLoad(
    $users,
    'user_id',      // 外键字段
    'orders',       // 关联表名
    'id'            // 本地键字段
...
// 批量加载用户资料
$users = QueryOptimizer::eagerLoadOne(
    $users,
    'user_id',
    'profiles',
    'id'
...
在 `app/useCode.php` 中配置：

```php
<?php
use Anon\Modules\Database\Database;
use Anon\Modules\System\Sharding;

Database\Sharding::init([
    'users' => [
        'shard_count' => 4,      // 分片数量
...
// 根据分片键自动获取表名
$userId = 12345;
$tableName = Database\Sharding::getTableName('users', $userId, 'id');
...
// 获取所有分片表名
$tables = Database\Sharding::getAllShardTables('users');
