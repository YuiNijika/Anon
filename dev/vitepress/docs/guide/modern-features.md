...
// 绑定接口到实现
$container->bind('UserRepositoryInterface', UserRepository::class);
...
use Anon\Modules\Http\MiddlewareInterface;
...
// 注册带别名的路由中间件
Middleware::alias('auth', 'AuthMiddleware');
Middleware::alias('throttle', 'ThrottleMiddleware');
```
...
try {
    // 路由处理代码...
    ResponseHelper::success(['message' => 'Success']);
} catch (Exception $e) {
...
// 设置缓存
Cache::set('user:1', $userData, 3600); // 1小时过期
Cache::set('config', $config, null); // 永不过期

// 获取缓存
$user = Cache::get('user:1');
$config = Cache::get('config', []); // 默认值
...
Console::command('cache:clear', function($args) {
    Cache::clear();
    Console::success('缓存已清空');
    return 0;
}, '清空缓存');

Console::command('route:list', function($args) {
    $routes = \Anon\Modules\System\Config::getRouterConfig()['routes'] ?? [];
    Console::info("已注册的路由:");
