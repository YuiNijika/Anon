...
Anon Framework 提供统一的附件处理系统 `Attachment`，支持文件上传、下载、删除、图片处理等功能。
...
```php
// 上传文件
$result = Attachment::upload('file', 'image');
if ($result['success']) {
...
// 获取附件信息
$attachment = Attachment::getAttachment(123);
 ...
 */
Attachment::upload(string $field = 'file', string $fileType = 'other'): array
```
...
```php
// 上传图片
$result = Attachment::upload('avatar', 'image');
 ...
 * @return array|null
 */
Attachment::getAttachment(int $id): ?array
```
 ...
 * @return bool
 */
Attachment::delete(int $id): bool
```

**示例：**

```php
if (Attachment::delete(123)) {
    echo "删除成功";
}
```
 ...
 */
Attachment::getAttachmentList(int $page = 1, int $pageSize = 20, ?string $mimeType = null, string $sort = 'new'): array
```
...
```php
// 获取图片列表
$result = Attachment::getAttachmentList(1, 20, 'image');
 ...
 * @return string
 */
Attachment::getUrl(int $id, string $fileType = 'other'): string
```

**示例：**

```php
$url = Attachment::getUrl(123, 'image');
...
```php
$mimeType = Attachment::detectMimeTypeFromFile('/path/to/file.jpg');
// 返回：image/jpeg
```
...
```php
if (Attachment::isExtensionMatchMimeType('jpg', 'image/jpeg')) {
    // 匹配成功
}
```
...
```php
$built = Attachment::buildRandomFilename('png');
...
```php
$fileType = Attachment::getFileTypeByMime('image/jpeg');
// 返回：'image'
```
...
```php
// 自动检测真实 MIME 类型
$realMimeType = Attachment::detectMimeTypeFromFile($tmpPath);

// 验证扩展名与 MIME 类型匹配
if (!Attachment::isExtensionMatchMimeType($ext, $realMimeType)) {
    Anon::error("文件类型不匹配");
}
```
...
foreach ($files['name'] as $key => $name) {
    ...
    $results[] = Attachment::upload('single', 'image');
}
```
