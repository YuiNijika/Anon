...
$data = Cache::remember('heavy:query', function() {
    // 这里是耗时操作，只会执行一次
...
class ArticleController
{
    ...
    {
        // 使用 remember 方法简化代码
        $article = Cache::remember("article:{$id}", function() use ($id) {
            return DB::table('articles')->find($id);
        }, 600);
        
        if (!$article) {
            return ResponseHelper::error('文章不存在', null, 404);
        }
