...
- 使用 `PascalCase`，大驼峰
- 示例：`Anon\Modules\System\Config`、`Anon\Modules\Http\RequestHelper`、`Anon\Modules\Database\UserRepository`

```php
use Anon\Modules\System\Config;
use Anon\Modules\Database\UserRepository;
```
...
2. **路由元数据**：使用 `RouterMeta` 常量配置路由属性
3. **异常处理**：使用 `try-catch` 包裹业务逻辑，统一使用 `Anon\Modules\Http\ResponseHelper::handleException()` 处理异常
...
try {
    ...
    ResponseHelper::handleException($e, '处理过程中发生错误');
}
```
...
- **统一使用**：`Anon\Modules\Http\ResponseHelper::handleException($e)` 处理异常
...
try {
    ...
    \Anon\Modules\Http\ResponseHelper::handleException($e, '登录处理过程中发生错误');
}
```

### 参数验证

- **验证必需参数**：使用 `Anon\Modules\Http\RequestHelper::validate()` 进行参数验证
...
- **登录检查**：使用 `Anon\Modules\Http\RequestHelper::requireAuth()` 进行登录检查
...
```php
\Anon\Modules\Http\RequestHelper::requireMethod('POST');
\Anon\Modules\Http\RequestHelper::requireMethod(['POST', 'PUT']);
```
...
- 所有用户输入必须验证和清理
- 使用 `Anon\Modules\Http\RequestHelper::validate()` 验证必需参数
...
- 使用 `Anon\Modules\Http\ResponseHelper` 统一响应格式
- 敏感信息不要输出到响应中

```php
// ✅ 正确：返回空数组而不是null
\Anon\Modules\Http\ResponseHelper::success([], '操作成功');

// ❌ 错误：返回null
\Anon\Modules\Http\ResponseHelper::success(null, '操作成功');
```
...
├── useApp.php              # 应用配置
├── useEnv.php              # 环境配置（常量）
├── useRouter.php           # 手动路由配置（可选）
└── useSQL.php              # SQL 安装配置

src/                        # 核心代码（PSR-4 / namespace）
└── Modules/                # 核心模块
```
...
- **使用方式**：`Anon\Modules\System\Env::get()` 获取配置值
...
- 使用 `Anon\Modules\Http\ResponseHelper` 统一响应格式
- 使用 `Anon\Modules\Http\RequestHelper` 统一请求处理
