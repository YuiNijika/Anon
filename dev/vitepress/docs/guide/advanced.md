...
$widget->register('my_widget', '我的组件', function($args) {
    echo '<div>' . Escape::html($args['title'] ?? '') . '</div>';
}, [
...
try {
    ...
    ResponseHelper::success($stats, '获取统计数据成功');
} catch (Exception $e) {
...
Hook::do_action('user_login', $user, $timestamp); // 多个参数
```
...
```php
// 添加过滤器钩子
Hook::add_filter('content_filter', function($content) {
    return str_replace('bad', '***', $content);
}, 10, 1);

// 应用过滤器
$filtered = Hook::apply_filters('content_filter', $content);
$filtered = Hook::apply_filters('content_filter', $content, $arg1, $arg2);
```

### 配置钩子

`config` 钩子用于扩展全局配置信息，在 `Config::getConfig()` 方法中应用：

```php
// 扩展配置字段
Hook::add_filter('config', function($config) {
...
```php
// 移除指定钩子
Hook::removeHook('user_login', $callback, 10);

// 移除所有钩子
Hook::removeAllHooks(); // 移除所有
Hook::removeAllHooks('user_login'); // 移除指定钩子的所有回调
Hook::removeAllHooks('user_login', 10); // 移除指定优先级

// 检查钩子是否存在
$exists = Hook::hasHook('user_login');
$priority = Hook::hasHook('user_login', $callback); // 返回优先级或false

// 获取当前执行的钩子名称
$currentHook = Hook::getCurrentHook();

// 获取钩子统计信息
$stats = Hook::getHookStats(); // 所有统计
$stats = Hook::getHookStats('user_login'); // 指定钩子统计

// 获取所有注册的钩子
$allHooks = Hook::getAllHooks();

// 清除统计信息
Hook::clearStats(); // 清除所有
Hook::clearStats('user_login'); // 清除指定钩子
```
...
```php
// 生成验证码（可自定义尺寸和长度）
$result = Captcha::generate(120, 40, 4); // 宽度、高度、长度
// 返回: ['image' => 'data:image/svg+xml;base64,...', 'code' => '1234']

// 验证验证码
$isValid = Captcha::verify($code);
// 返回: bool

// 区分大小写验证（默认不区分）
$isValid = Captcha::verify($code, true);
