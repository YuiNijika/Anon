...
完整的访问日志系统说明，包括启用方法、配置选项、使用场景、性能优化等，请参考 [访问日志系统](../access-log.md)。
...
请求参数和响应格式详见 [访问日志系统 - API 接口](../access-log.md#api-接口)。  

### 路由注册与元数据

管理路由由 `Admin::initRoutes()` 注册，可通过 `Admin::addRoute()` 扩展。元数据支持：`requireLogin`、`requireAdmin`、`method`、`token`。全局配置可通过 `Config::getConfig()` 获取，并由 `config` 钩子扩展。
