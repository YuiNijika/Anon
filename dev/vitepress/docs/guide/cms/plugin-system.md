...
// 从 options 表读取
$options = Plugin::getPluginOptions('my-plugin');
...
只需继承 `Plugin\Base`，无需手动编写 `init` 加载逻辑：

```php
<?php
if (!defined('ANON_ALLOWED_ACCESS')) exit;

class Plugin\AppList extends Plugin\Base
{
...
使用 `Navbar::mount` 辅助方法，可以将菜单项插入到现有的分组中（如 `manage` 管理, `settings` 设置）。

```php
Anon::filter('admin_navbar_sidebar', function ($items) {
    // 将菜单挂载到 "manage" (管理) 组下
    Navbar::mount($items, 'manage', [
...
插件名称从目录名获取，类名不区分大小写，系统自动匹配。目录 HelloWorld、helloworld 均对应类名 Plugin\HelloWorld。主入口文件名 Index.php 或 index.php 等均可识别。
...
- 调用插件的 `init()` 方法
- 在 `init()` 方法内可通过 `Plugin::isApiMode()` 或 `Plugin::isCmsMode()` 判断当前模式
- 执行插件注册的路由和钩子
...
{
    // 添加动作钩子
    Hook::add_action('some_action', function() {
        // 执行逻辑
    });

    // 添加过滤器钩子
    Hook::add_filter('some_filter', function($value) {
...
在插件主类中实现静态方法 `getSettingsSchema()`，返回字段名到定义的映射，含类型、标签、默认值等。继承 `Plugin\Base` 时，实例方法 `$this->options()` 为选项代理，与静态 schema 方法同名不冲突。
...
插件可继承 `Plugin\Base`，使用实例方法 `$this->options()` 参与统一选项优先级。
...
class Plugin\HelloWorld extends Plugin\Base
{
...
public static function init()
{
    ...
    // 获取当前模式字符串
    $mode = Plugin::getAppMode(); // 返回 'api' 或 'cms'
}
```

### 辅助方法说明

- `Plugin::isApiMode()`: 判断是否为 API 模式，返回 `bool`
- `Plugin::isCmsMode()`: 判断是否为 CMS 模式，返回 `bool`
- `Plugin::getAppMode()`: 获取当前应用模式，返回 `'api'` 或 `'cms'`

## 注意事项

1. 插件目录名与主入口文件名 Index.php 不区分大小写，类名也不区分大小写，系统自动匹配
2. 插件必须实现 `init()` 方法
3. 插件必须放在 `app/Plugin/` 目录下
...
class Plugin\UserStats
{
    public static function init()
    {
        ...
        // 注册钩子
        Hook::add_action('user_login', [self::class, 'onUserLogin']);
    }
