...
**文件位置：** `app/Theme/{themeName}/app/code.php`
...
- **$this->post()** / **$this->page()** — 当前文章/页面对象（`Cms\Post`），无则 null
- **$this->user()** — 当前登录用户（`Cms\User`），未登录为 null  
  - `$this->user()->uid()`、`name()`、`email()`、`displayName()`、`avatar()`、`url()`（作者页链接）、`get($key, $default)`
- **$this->theme()** — 主题辅助（`Theme\Helper`）  
...
**示例文件位置：** `app/Theme/Default/app/code.php`
...
// 注册过滤器钩子
Hook::add_filter('theme_page_title', function ($title) {
    $siteTitle = Options::get('site_title', '');
    return $siteTitle ? "{$title} - {$siteTitle}" : $title;
});

// 注册自定义路由
Config::addRoute('/theme/custom', function () {
    ResponseHelper::success(['setting' => Options::get('custom_setting', 'default')], '获取主题设置成功');
});
```
...
- 值为 JSON 对象
- 写入逻辑：有该 name 则更新 value，无则插入，与 Options::set 同款。主题层封装了 Options::setStorage，单键设置用 set 或 setMany 时内部会调用 setStorage 保证持久化。
...
$siteTitle = Options::get('site_title', '默认标题');
```
...
`$this->options()` 返回统一选项代理 `OptionsProxy`。主题内默认优先级：theme > plugin > system。
...
- `Options` 可以使用 `ThemeOptions` 别名
- `Theme` 可以使用 `CmsTheme` 别名
...
```php
Options::set('site_url', 'https://example.com');
```
...
```php
Theme::render(string $templateName, array $data = []): void
```
...
```php
$this->profileUser(): ?Cms\User
```

**返回值：**

- 成功解析用户返回 `Cms\User` 对象
- 否则返回 `null`
...
// 添加新的 MIME 类型
Hook::add_filter('cms_theme_mime_types', function($mimeTypes) {
...
// 修改文件类型到目录的映射
Hook::add_filter('cms_theme_type_dirs', function($typeDirs) {
...
在模板文件中定义 `PageMeta` 常量来设置页面 SEO 信息：
...
系统提供了类似 Typecho 的链式调用 API，通过 `Cms\Post` 对象访问文章和页面数据。
...
Theme::render('layout', compact('title', 'content'));
?>
```
...
```php
<?php
echo "当前主题: " . Theme::getCurrentTheme();
?>
```
