...
class RESTful\XXX
{
    ...
    {
        try {
            $page = max(1, intval(RequestHelper::input('page', 1)));
            $pageSize = min(100, max(1, intval(RequestHelper::input('page_size', 10))));
            
            $result = Services\XXX::getList($page, $pageSize);
            ...
            return ResponseHelper::error('获取列表失败: ' . $e->getMessage(), 500);
        }
    }
    ...
    {
        try {
            $item = Services\XXX::getById($id);
            
            if (!$item) {
                return ResponseHelper::error('记录不存在', 404);
            }
            
            return ResponseHelper::success($item);
        } catch (Exception $e) {
            return ResponseHelper::error('获取失败: ' . $e->getMessage(), 500);
        }
    }
    ...
    public static function store(): array
    {
        try {
            ...
            // 验证必填字段
            if (empty($data['name'])) {
                return ResponseHelper::error('名称不能为空', 400);
            }
            
            $item = Services\XXX::create($data);
            
            return ResponseHelper::success($item, '创建成功', 201);
        } catch (Exception $e) {
    ...
    public static function update(int $id): array
    {
        try {
            ...
            $item = Services\XXX::update($id, $data);
            
            return ResponseHelper::success($item, '更新成功');
        } catch (Exception $e) {
            return ResponseHelper::error($e->getMessage(), 400);
        }
    }
...
public static function init()
{
    ...
    self::addRoute($routePrefix . '/xxx', function () use ($tokenRequired) {
        if (!Options::get('api_enabled', '0')) {
            ResponseHelper::error('RESTful API 未启用，请在后台设置中启用', 403);
            return;
        }
        ...
        if ($requestMethod === 'GET') {
            ...
            if (!RequestHelper::isAdmin()) {
                ResponseHelper::error('需要管理员权限', 403);
                return;
            }
            RESTful_XXX::store();
        } else {
            ResponseHelper::error('不支持的请求方法', 405);
        }
    }, [
        'method' => ['GET', 'POST'],
        'token' => $tokenConfig,
    ...
    self::addRoute($routePrefix . '/xxx/{id}', function ($params) use ($tokenRequired) {
        ...
        if ($id <= 0) {
            ResponseHelper::error('ID 无效', 400);
            return;
        }
        ...
        if ($requestMethod === 'GET') {
            ...
            RESTful\XXX::update($id);
        } elseif ($requestMethod === 'DELETE') {
            if (!RequestHelper::isAdmin()) {
                ResponseHelper::error('需要管理员权限', 403);
                return;
            }
            RESTful\XXX::destroy($id);
        } else {
            ResponseHelper::error('不支持的请求方法', 405);
        }
    }, [
...
```php
// 成功响应
ResponseHelper::success($data, '操作成功', 200);

// 错误响应
ResponseHelper::error('错误信息', 400);
```
...
// 文章创建前修改数据
Hook::add_filter('cms_post_before_insert', function($data, $context) {
    // 自动添加前缀
...
// 修改 API 请求参数
Hook::add_filter('restful_post_list_params', function($params) {
    // 限制每页最大数量
...
// 修改 API 响应数据
Hook::add_filter('restful_post_list_response', function($response) {
    // 添加自定义字段
...
// 文章创建前验证
Hook::add_filter('restful_post_before_create', function($data) {
    // 自定义验证逻辑
...
// 文章删除前检查
Hook::add_action('restful_post_before_delete', function($id, $post) {
    // 检查是否有依赖关系
